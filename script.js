...
function handleClick() {
    ...
    if (!bestTime || reactionTime < bestTime) {
        ...
        bestTimeDisplay.textContent = `Best Time: ${(bestTime / 1000).toFixed(2)}s`; // Convert to seconds
    }
    
    // Update UI
    targetBox.style.backgroundColor = '#2ecc71';
    targetBox.textContent = '✓';
    currentTimeDisplay.textContent = `Your reaction time: ${(reactionTime / 1000).toFixed(2)}s`; // Convert to seconds
    attemptsDisplay.textContent = attempts;
    averageTimeDisplay.textContent = (averageTime / 1000).toFixed(2); // Convert to seconds
